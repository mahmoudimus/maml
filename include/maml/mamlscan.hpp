...
namespace maml {
    namespace locate {
        ...
        struct Hit {
            size_t offset = 0; // where the pattern matched
            size_t value = 0; // the requested save slot
            size_t candidates = 0; // seed hits considered
            size_t verified = 0; // how many reached the matcher
        };

        // A pattern parsed once, plus the seed chosen for one image. Seed choice
        // is a property of the (pattern, image) pair, so a Compiled primed for
        // image A must be re-primed for image B.
        struct Compiled {
        ...
        inline Compiled compile(std::string_view pattern) {
            Compiled c;
            c.pat = compiler::optimize_pattern(compiler::parse_pattern(std::string(pattern)));
            c.fx  = fixed_bytes(pattern);
            return c;
        }

        inline void prime(Compiled& c, std::span<const uint8_t> image, size_t cap = kSeedCapExact) {
            c.seed = select_seed(image, c.fx, cap);
        }

        namespace detail {
            // The seeded scan shared by `find` and `find_all`: scan_literal step
            // over the seed run, reject candidates that can't be a real match (the
            // `at < c.seed.offset` guard and the span-past-end-of-image guard),
            // then re-check every fixed byte before calling back.
            //
            // `candidates` bumps right after the two offset guards; `verified`
            // bumps right after the fixed-byte re-check passes -- the same two
            // points both callers bumped before this was extracted. Each caller
            // owns its own counters by reference, so the two scans stay
            // independently observable even though the loop body is now shared.
            //
            // `on_verified(start)` runs once per candidate that clears the
            // fixed-byte re-check; returning true stops the scan.
            template <typename F>
            inline void seeded_scan(std::span<const uint8_t> image, const Compiled& c,
                             size_t& candidates, size_t& verified, F&& on_verified) {
                const uint8_t* first = image.data();
                const uint8_t* last = image.data() + image.size();
                scan_literal(first, last, c.seed.bytes.data(), c.seed.bytes.size(),
                    [&](const uint8_t* q) -> bool {
                        const size_t at = static_cast<size_t>(q - first);
                        if (at < c.seed.offset)
                            return false;
                        const size_t start = at - c.seed.offset;
                        if (start + c.fx.span > image.size())
                            return false;
                        ++candidates;
                        for (size_t j = 0; j < c.fx.offsets.size(); ++j)
                            if (first[start + c.fx.offsets[j]] != c.fx.bytes[j])
                                return false;
                        ++verified;
                        return on_verified(start);
                    });
            }
        } // namespace detail

        // First match of an already-compiled pattern. `save_index` selects the
        // saved cursor: 0 is the match position, 1 the first `'`.
        inline std::optional<Hit> find(std::span<const uint8_t> image,
            const Compiled& c,
            size_t save_index = 0) {
            ImageMatchTarget target(image, 0);
            Hit hit;
            bool found = false;

            auto verify = [&](size_t start) -> bool {
                BinaryMatcher<> m(c.pat, target, start, 1);
                auto h = m.next_match();
                if (!h || h->size() <= save_index)
                    return false;
                hit.offset = start;
                hit.value = (*h)[save_index];
                found = true;
                return true;
            };

            if (!c.seed.ok()) {
                BinaryMatcher<> m(c.pat, target);
                auto h = m.next_match();
                if (!h || h->size() <= save_index)
                    return std::nullopt;
                hit.offset = (*h)[0];
            ...
            detail::seeded_scan(image, c, hit.candidates, hit.verified,
                [&](size_t start) { return verify(start); });
            return found ? std::optional<Hit>(hit) : std::nullopt;
        }

        // Every match, not just the first. `limit == 0` is unbounded.
        //
        // Hit::candidates and Hit::verified accumulate over the scan, so each
        // element carries the running totals as of the moment it was verified,
        // and the last element carries the totals as of the last SUCCESSFUL
        // match -- not necessarily the totals for the whole scan. A candidate
        // rejected after the final match still bumps the counters without
        // producing an element in `out`, so the running totals can keep moving
        // past the point the last `Hit` was recorded. Hit is left alone
        // deliberately: mamlscan reads those fields today.
        //
        // `verified` increments as soon as the fixed-byte re-check passes, the
        // same point `find` increments it (mamlscan.hpp, the Compiled overload of
        // `find`) -- not after the matcher confirms the full pattern. Hit's own
        // comment documents the field as "how many reached the matcher", and a
        // candidate that fails the matcher still reached it. `candidates` and
        // `verified` are therefore not generally equal: a seed hit can pass the
        // fixed-byte prefilter (bumping `verified`) and still fail the matcher's
        // remaining checks, without producing an element in `out`.
        inline std::vector<Hit> find_all(std::span<const uint8_t> image,
            const Compiled& c,
            size_t save_index = 0,
            size_t limit = 0) {
            std::vector<Hit> out;
            ...
            if (!c.seed.ok()) {
                BinaryMatcher<> m(c.pat, target);
                while (auto h = m.next_match()) {
                    if (h->size() <= save_index)
                        continue;
                    Hit hit;
                    hit.offset = (*h)[0];
                    hit.value = (*h)[save_index];
            ...
            detail::seeded_scan(image, c, candidates, verified,
                [&](size_t start) {
                    take(start);
                    return limit && out.size() >= limit;
                });
            return out;
        }

        // Unchanged contract: returns nullopt rather than throwing when the
        // pattern does not parse, and seeds with kSeedCapLoadTime because this is
        // the overload the DLL calls at load time.
        inline std::optional<Hit> find(std::span<const uint8_t> image,
            ...
            if (seed && seed->ok())
                ...
                prime(c, image, kSeedCapLoadTime);
            return find(image, c, save_index);
        }
