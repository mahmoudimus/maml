...
def test_loose_is_func_alias_and_trace_names_modifier():
    image=v1.Image(bytes.fromhex('90 AA AA'),functions=[Function(0, [(0, 3)])])
...
def test_strict_rejects_interior_even_when_entry_is_present(pattern):
    image=v1.Image(bytes.fromhex('90 90'),functions=[Function(0, [(0, 2)])])
...
def test_strict_accepts_entries_and_projected_addresses_with_base():
    image=v1.Image(bytes.fromhex('AA 90 AA 90'),base=0x1000,functions=[Function(0, [(0, 2)]), Function(2, [(2, 4)])])
...
def test_strict_rejects_uncovered_addresses_but_loose_drops_them():
    image=v1.Image(bytes.fromhex('AA BB'),functions=[Function(0, [(0, 1)])])
...
def test_empty_input_and_missing_metadata_are_distinct():
    ...
    assert not pipeline.run(v1.Image(b'\x90',functions=[Function(0, [(0, 1)])])).ok
    with pytest.raises(v1.ExecutionError,match='function metadata'):
